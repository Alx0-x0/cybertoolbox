...
async function getIPData(ipAddress = '') {
    ...
    if (!success) showError(lastError ? lastError.message : t('tool.geo.locate_error', 'Unable to locate IP.'));
    setLoading(false);
}
...
function updateMap(lat, lon, city) {
    if (lat && lon) {
        ...
        marker.bindPopup(`<b>${city || t('tool.geo.location', 'Localisation')}</b><br>Lat: ${lat}, Lon: ${lon}`).openPopup();
...
function showError(message) {
    dom.errorMsg.textContent = `${t('tool.common.error_prefix', 'Erreur :')} ${message}`;
