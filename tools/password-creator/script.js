...
function updateStrength(password) {
    ...
    let text = t('tool.password.weak', 'Faible');
    
    if (entropy > 120) { strength = 100; color = '#3fb950'; text = t('tool.password.excellent', 'Excellent'); }
    else if (entropy > 80) { strength = 75; color = '#00ff41'; text = t('tool.password.strong', 'Fort'); }
    else if (entropy > 50) { strength = 50; color = '#d29922'; text = t('tool.password.medium', 'Moyen'); }
    else { strength = 25; }
    
    bar.style.width = strength + '%';
    bar.style.backgroundColor = color;
    label.innerHTML = `${t('tool.password.strength', 'Force')}: <span style="color:${color}">${text}</span> (${Math.round(entropy)} bits)`;
    ...
    crackTimeLabel.innerHTML = `<i class="fa-solid fa-server"></i> ${t('tool.password.crack_time', 'Craquage estimé (Supercalculateur)')} : <span style="color:${color}">${formatTime(seconds)}</span>`;
}

function formatTime(seconds) {
    if (seconds < 1) return t('tool.password.instant', 'Instant');
    if (seconds < 60) return Math.round(seconds) + " " + t('tool.password.seconds', 'seconds');

    const minutes = seconds / 60;
    if (minutes < 60) return Math.round(minutes) + " " + t('tool.password.minutes', 'minutes');

    const hours = minutes / 60;
    if (hours < 24) return Math.round(hours) + " " + t('tool.password.hours', 'hours');

    const days = hours / 24;
    if (days < 365) return Math.round(days) + " " + t('tool.password.days', 'days');

    const years = days / 365;
    if (years < 1000) return Math.round(years) + " " + t('tool.password.years', 'years');
    if (years < 1000000) return (years / 1000).toFixed(1) + " " + t('tool.password.millennia', 'millennia');

    return t('tool.password.eons', 'Eons (+1M years)');
}
...
async function checkLeak() {
    ...
    resultBox.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${t('tool.password.checking', 'Vérification...')}`;
    ...
    if (!hash) {
        resultBox.textContent = t('tool.password.hash_error', "Erreur : Impossible de calculer le hash (Contexte non sécurisé ?)");
        return;
    }
    ...
    try {
        ...
        if (match) {
            ...
            resultBox.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> ${t('tool.password.pwned_seen_prefix', 'Ce mot de passe a été vu')} <strong>${parseInt(count).toLocaleString()}</strong> ${t('tool.password.pwned_seen_suffix', 'fois dans des fuites de données !')}`;
            resultBox.classList.add('danger');
        } else {
            resultBox.innerHTML = `<i class="fa-solid fa-shield-check"></i> ${t('tool.password.not_found', "Ce mot de passe n'a pas été trouvé dans la base de données publique.")}`;
            ...
            if (password === generatedPassword) {
                pwnedStatusSpan.innerHTML = `<i class="fa-solid fa-shield-check"></i> ${t('tool.password.not_found_short', 'Non trouvé')}`;
                pwnedStatusSpan.style.display = 'inline';
            }
        }
    } catch (e) {
        resultBox.textContent = t('tool.password.api_error', "Erreur de connexion à l'API.");
    }
}
